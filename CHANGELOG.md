...
- **Subpage false-positives** (#6): `Plugin::addBodyClass()` and `AdminPage::matchPage()` used `str_contains($screen->id, $page['menu_slug'])`, which matched any admin screen whose ID contained a Wireframe page's slug as a substring. Sibling subpages registered under the same parent (e.g. `example-plugin_page_example-plugin-conversion` next to the Wireframe `example-plugin` page) were incorrectly receiving the `wireframe-admin` body class (and its `#wpcontent { padding-left: 0 }` reset) and triggering framework asset enqueue. Both lookups now anchor on the `_page_{menu_slug}` suffix, which the underscore-bounded `_page_` separator keeps unambiguous.
- **`DateField` rendered raw ISO timestamp when the site date format was empty** (#3, thanks to the external contributor): when WP's Settings → General → Date Format is set to "Custom" with an empty value, `wp.date.getSettings().formats.date` returns `''`; `dateI18n` then echoed the stored ISO 8601 string back into the picker button instead of a `YYYY-MM-DD` label. `DateField` now falls back to a sensible default format when the site setting is empty.
